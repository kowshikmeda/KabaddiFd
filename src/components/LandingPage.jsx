...
const LandingPage = () => {
      ...
      try {
        ...
        const transformedData = response.data.data.map(match => ({
          ...
          team1: {
            ...
            photo: match.team1Photo || `https://ui-avatars.com/api/?name=${match.team1Name.split(' ').join('+')}&background=random`,
            score: match.team1Score
          },
          team2: {
            name: match.team2Name,
            photo: match.team2Photo || `https://ui-avatars.com/api/?name=${match.team2Name.split(' ').join('+')}&background=random`,
          ...
          // --- CHANGE 1: Store the raw remainingDuration in seconds ---
          remainingDuration: match.remainingDuration 
        }));
  ...
  // --- CHANGE 2: useEffect hook to handle the live countdown timer ---
  useEffect(() => {
    // Start an interval only if there is at least one LIVE match
    const hasLiveMatch = matches.some(match => match.status === 'LIVE');
    if (!hasLiveMatch) {
      return; // No need for a timer if no matches are live
    }

    const timerInterval = setInterval(() => {
      setMatches(prevMatches =>
        prevMatches.map(match => {
          // If the match is LIVE and has time remaining, decrement the duration
    ...
    // or when the `matches` state changes, to prevent memory leaks.
    return () => clearInterval(timerInterval);
  }, [matches]); // Rerun this effect if the matches data changes
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="container mx-auto px-6 pt-8 pb-12">
        <div className="max-w-2xl mx-auto">
          {/* --- CHANGE 3: Add 'paused' to the filter array --- */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {filteredMatches.length > 0 ? (
              filteredMatches.map((match) => {
                ...
                return (
                  <div key={match.id} onClick={() => navigate(`/scorecard/${match.id}`)} className="cursor-pointer bg-white/5 backdrop-blur-lg rounded-3xl p-6 border border-white/10 transition-all duration-500 hover:border-orange-500/50 hover:shadow-2xl hover:shadow-orange-500/20">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      {(match.status === 'LIVE' || match.status === 'PAUSED') && (
                        <div className={`flex items-center space-x-2 ${match.status === 'LIVE' ? 'text-orange-400' : 'text-yellow-400'}`}>
                          <Clock className="w-4 h-4" />
                          {/* --- CHANGE 4: Format the dynamic remainingDuration for display --- */}
